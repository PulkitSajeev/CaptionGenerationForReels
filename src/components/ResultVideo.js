...
export default function ResultVideo({filename, transcriptionItems}) {
    const videoUrl = "https://captiongeneratorbucket.s3.amazonaws.com/"+filename
    ...
    const load = async () => {
        ...
        ffmpeg.writeFile('/tmp/roboto.ttf', await fetchFile(roboto));
    ...
    return (
        ...
        <div className="mb-4">
                        <button 
                            onClick={transcode}
                            className="bg-green-600 py-2 px-6 rounded-full inline-flex gap-2 border-2 border-puprple-700/50 cursor-pointer">
                            <SparklesIcon/>
                    ...
                    <div>
                        Primary Colour:
                        <input type="color" value={primaryColour} onChange={ev => setPrimaryColour(ev.target.value)}/>
                        <br/>
                        Outline Colour:
                        <input type="color" value={OutlineColour} onChange={ev => setOutlineColour(ev.target.value)}/>
                    </div>
